...
sealed class CalculatorState {
    ...
    data class AddOperation(val lhs: Int, val rhs: Int?) : CalculatorState() {
        override val value get() = lhs + (rhs ?: 0)
    }

    data class SubtractOperation(val lhs: Int, val rhs: Int?) : CalculatorState() {
        override val value get() = lhs - (rhs ?: 0)
    }
}
    ...
    private fun deleteOne() = setState {
        copy(
            calculatorState = when (calculatorState) {
                ...
                is AddOperation -> if (calculatorState.rhs == null) {
                    SingleValue(calculatorState.lhs)
                } else {
                    if (calculatorState.rhs.withLastDigitRemoved == 0) {
                        AddOperation(calculatorState.lhs, null)
                    } else {
                        AddOperation(calculatorState.lhs, calculatorState.rhs.withLastDigitRemoved)
                    }
                }
                is SubtractOperation -> if (calculatorState.rhs == null) {
                    SingleValue(calculatorState.lhs)
                } else {
                    if (calculatorState.rhs.withLastDigitRemoved == 0) {
                        SubtractOperation(calculatorState.lhs, null)
                    } else {
                        SubtractOperation(calculatorState.lhs, calculatorState.rhs.withLastDigitRemoved)
                    }
                }
            }
        )
